...
class AnnotationConfigurationPass implements CompilerPassInterface
{
    ...
    private function getBundleFiles( ContainerBuilder $container ): array
    {
        ...
        foreach ( $bundles as $name => $bundle ) {
            ...
            $finder = new Finder();
            $finder->files()->in( $bundle['path'] )->name( '*.php' )->contains( 'Vankosoft\ThruwayBundle\Annotation' )->depth( '< 5' );
        ...
        $finder = new Finder();
        try {
            $finder->files()->in( $controllerDir )->name( '*.php' )->contains( 'Vankosoft\ThruwayBundle\Annotation' )->depth( '< 5' );
